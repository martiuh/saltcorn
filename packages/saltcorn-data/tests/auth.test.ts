...
const createPersonsTable = async () => {
  const persons = await Table.create("TableOwned17");
  await Field.create({
    table: persons,
    name: "lastname",
    type: "String",
...
describe("updateWhereWithOwnership", () => {
  ...
  it("should update where with inherit", async () => {
    const persons = await createPersonsTable();
    const tasks = await Table.create("PersonTasks");
    await Field.create({
      table: tasks,
      name: "name",
      type: "String",
    ...
    await Field.create({
      table: tasks,
      name: "person",
      type: `Key to ${persons.name}`,
    });
    await persons.update({ ownership: { field: "owner" } });
    await tasks.update({ ownership: { inherit: "person" } });

    await persons.insertRow({ lastname: "Joe", age: 12 });
    await persons.insertRow({ lastname: "Sam", age: 13, owner: 1 });
    const w = {};
    const fields = await tasks.getFields();
    tasks.updateWhereWithOwnership(w, fields, non_owner_user);
    expect(w).toEqual({
      person: {
        inSelect: { field: "id", table: persons.name, where: { owner: 6 } },
      },
    });
    await tasks.delete();
    await persons.delete();
